...
void draw() {
	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // Both buffers have to be cleared for the wireframe mode
	// Load identity matrix
	glLoadIdentity();

	// Set wireframe mode if enabled
	// It also affects the view from the portals
	// Without this the portals work with normals, textures and shaders
	// With this the view from the portals is also in wireframe
    if (wireframe_mode) {
...
void drawOverlay() {
	...
	// If game is not paused
		...
		if(player.getState() == PS_ALIVE) {
			...
			glColor4f(1.f, 1.f, 1.f, 1.f);
			glBegin(GL_QUADS);
				glTexCoord2f(0,0); glVertex2f(width/2-40, height/2-30);
				glTexCoord2f(0,1); glVertex2f(width/2-40, height/2+50);
				glTexCoord2f(1,1); glVertex2f(width/2+40, height/2+50);
				glTexCoord2f(1,0); glVertex2f(width/2+40, height/2-30);
			glEnd();
		}
...
void key_down(unsigned char key, int x, int y) {
	...
	else if (key == 'n') { // Toggle wireframe mode
